...
- (void)viewDidLoad {
    ...
    picker = [[UIPickerView alloc] initWithFrame:CGRectMake(0, 40, self.view.frame.size.width, 200)];
    picker.dataSource = self;
    picker.delegate = self;
    picker.showsSelectionIndicator = YES;
    UILabel *hourLabel = [[UILabel alloc] initWithFrame:CGRectMake(44, picker.frame.size.height / 2 - 15, 75, 30)];
    hourLabel.textColor = [UIColor whiteColor];
    hourLabel.text = @"Hours";
    [picker addSubview:hourLabel];
    UILabel *minsLabel = [[UILabel alloc] initWithFrame:CGRectMake(44 + (picker.frame.size.width / 3), picker.frame.size.height / 2 - 15, 75, 30)];
    minsLabel.textColor = [UIColor whiteColor];
    minsLabel.text = @"Minutes";
    [picker addSubview:minsLabel];
    UILabel *secsLabel = [[UILabel alloc] initWithFrame:CGRectMake(43 + ((picker.frame.size.width / 3) * 2), picker.frame.size.height / 2 - 15, 75, 30)];
    secsLabel.textColor = [UIColor whiteColor];
    secsLabel.text = @"Seconds";
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view
{
    ...
    columnView.font = [UIFont boldSystemFontOfSize:18];
